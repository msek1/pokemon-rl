...
class Trainer:
    def __init__(self, lr = 0.001, gamma = 0.98, battles_per_epoch = 100, reward_policy: RewardPolicy=RewardPolicy()) -> None:
        self.lr = lr
        self.gamma = gamma
    ...
    def calculate_all_discounted_reward(self, p: RLBot):
        return {tag: self.calculate_reward_for_battle(battle) for (tag, battle) in p.battles.items()}
    
    def calculate_reward_for_battle(self, battle: AbstractBattle):
        rewards = [
        ...
        discounted = [r * self.gamma ** i for (i,r) in enumerate(rewards)]
